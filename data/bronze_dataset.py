...
class BronzeWare(Dataset):
    def __init__(self, X, Y, X_xml=None, Y_shape=None, transform=None, indices=None):
        self.X_img = X[indices] if indices is not None else X
        self.Y_age = Y[indices] if indices is not None else Y
        self.X_xml = X_xml[indices] if indices is not None else X_xml
        self.Y_shape = Y_shape[indices] if indices is not None else Y_shape
    ...
    def __getitem__(self, index):
        ...
        with open(self.X_xml[index], encoding='utf-8') as fid:
            xml_str = fid.read()
        ...
        level_1 = dating_tree_dict[str(self.Y_age[index])][0] - 1
        level_2 = dating_tree_dict[str(self.Y_age[index])][1] - 1
        labels_level1.append(level_1)
        labels_level2.append(level_2)
        shape_label.append(float(self.Y_shape[index]))
        ...
        img = Image.open(self.X_img[index]).convert('RGB')
        ...
        return {
            ...
            'y': labels_level2[0],  # (B,)
            'index': index,  # (B,)
            'shape_label': shape_label[0],  # (B,)
            'labels_level1': labels_level1[0],  # (B,)
            'attribute_label': attributes,  # (B, 96)
        }
